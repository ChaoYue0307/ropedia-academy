...
# 3D Gaussian Splatting  🚧 not trained yet

> Turn a set of your own photos into a real 3D Gaussian-Splatting scene (CUDA rasterizer + COLMAP).
...
| | |
|---|---|
| **Base model** | From scratch — per-scene optimization |
| **Task** | 3D scene reconstruction (photos → splats) |
| **Training objective** | Photometric loss over differentiably-rasterized 3D Gaussians (+ densification). |
| **Track** | B · 3D & rendering |
| **Built on** | [graphdeco-inria/gaussian-splatting](https://github.com/graphdeco-inria/gaussian-splatting) |
| **Notebook** | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/B_gaussian_splatting_3d.ipynb) |
| **Compute / storage / time** | GPU required — see the *Compute · storage · time* table in the notebook |

## Dataset
- **Source:** Your photos + COLMAP camera poses.
...
## Evaluation results
⏳ **Pending** — run the notebook on a GPU to fill this in. This lab reports **PSNR · SSIM · LPIPS (held-out views)** on a held-out split (see its *Evaluate* cell).

## Inference example
No weights are published yet. After a GPU run, load the checkpoint/adapter the notebook saves (it also has a ready inference cell). Base model: **From scratch — per-scene optimization**.

## How to fill this repo
1. Open the [notebook in Colab](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/B_gaussian_splatting_3d.ipynb) → **Runtime → GPU → Run all** (runs the real pipeline).
...
## License
Code: **MIT** (this repository). The **base model** ([graphdeco-inria/gaussian-splatting](https://github.com/graphdeco-inria/gaussian-splatting)) and **dataset** are each under their own licenses — check the upstream source before redistribution.
...
**Method / original work:** Kerbl et al., *3D Gaussian Splatting*, SIGGRAPH 2023.

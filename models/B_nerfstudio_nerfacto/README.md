...
# Nerfstudio nerfacto  🚧 not trained yet
...
| | |
|---|---|
| **Base model** | From scratch — per-scene (nerfacto / splatfacto) |
| **Task** | NeRF / Gaussian scene from video |
| **Training objective** | Volumetric photometric loss with proposal sampling + hash encoding. |
...
## Dataset
- **Source:** Your phone video / image set.
...
## Evaluation results
⏳ **Pending** — run the notebook on a GPU to fill this in. This lab reports **PSNR · SSIM · LPIPS** on a held-out split (see its *Evaluate* cell).

## Inference example
No weights are published yet. After a GPU run, load the checkpoint/adapter the notebook saves (it also has a ready inference cell). Base model: **From scratch — per-scene (nerfacto / splatfacto)**.

## How to fill this repo
1. Open the [notebook in Colab](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/B_nerfstudio_nerfacto.ipynb) → **Runtime → GPU → Run all** (runs the real pipeline).
...
## License
Code: **MIT** (this repository). The **base model** ([nerfstudio-project/nerfstudio](https://docs.nerf.studio/)) and **dataset** are each under their own licenses — check the upstream source before redistribution.

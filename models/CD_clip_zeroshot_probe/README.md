...
# CLIP: zero-shot vs. probe  🚧 not trained yet

> CLIP zero-shot classification + a trained linear probe on frozen features.
...
| | |
|---|---|
| **Base model** | open_clip ViT-B/32 (pretrained) |
| **Task** | vision-language probing |
| **Training objective** | Zero-shot classification via text prompts + a trained linear probe on frozen features. |
| **Track** | C · Egocentric vision |
| **Built on** | [open_clip](https://github.com/mlfoundations/open_clip) |
| **Notebook** | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/training/CD_clip_zeroshot_probe.ipynb) |
...
## Evaluation results
⏳ **Pending** — run the notebook on a GPU to fill this in. This lab reports **zero-shot accuracy · linear-probe accuracy** on a held-out split (see its *Evaluate* cell).

## Inference example
No weights are published yet. After a GPU run, load the checkpoint/adapter the notebook saves (it also has a ready inference cell). Base model: **open_clip ViT-B/32 (pretrained)**.

## How to fill this repo
1. Open the [notebook in Colab](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/training/CD_clip_zeroshot_probe.ipynb) → **Runtime → GPU → Run all** (runs the real pipeline).
...
## License
Code: **MIT** (this repository). The **base model** ([open_clip](https://github.com/mlfoundations/open_clip)) and **dataset** are each under their own licenses — check the upstream source before redistribution.
...
**Method / original work:** Radford et al., *CLIP*, ICML 2021.

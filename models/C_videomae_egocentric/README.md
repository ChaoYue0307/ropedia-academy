...
# VideoMAE — egocentric fine-tune  🚧 not trained yet
...
| **Training objective** | Supervised video-clip classification (fine-tune the pretrained ViT). |
...
## Dataset
- **Source:** EPIC-Kitchens-100 / Ego4D (demo: UCF101 subset).
...
## Evaluation results
⏳ **Pending** — run the notebook on a GPU to fill this in. This lab reports **top-1 / top-5 accuracy** on a held-out split (see its *Evaluate* cell).
...
1. Open the [notebook in Colab](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/C_videomae_egocentric.ipynb) → **Runtime → GPU → Run all** (runs the real pipeline).
...
## License
Code: **MIT** (this repository). The **base model** ([EPIC-Kitchens / Ego4D (🤗 transformers)](https://huggingface.co/docs/transformers/en/tasks/video_classification)) and **dataset** are each under their own licenses — check the upstream source before redistribution.

...
# QLoRA — fine-tune an LLM  🚧 not trained yet

> Instruction-tune a small LLM on your own data cheaply with 4-bit QLoRA.
...
| | |
|---|---|
| **Base model** | Qwen/Qwen2.5-0.5B-Instruct in 4-bit (swap any base) |
| **Task** | instruction fine-tuning (4-bit LoRA) |
| **Training objective** | Supervised instruction fine-tuning of LoRA adapters on a 4-bit base. |
| **Track** | LM · Language & multimodal |
| **Built on** | [huggingface/trl + peft](https://github.com/huggingface/trl) |
| **Notebook** | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/LM_qlora_finetune_llm.ipynb) |
| **Compute / storage / time** | GPU required — see the *Compute · storage · time* table in the notebook |

## Dataset
- **Source:** mlabonne/guanaco-llama2-1k (or your own chat data).
...
## Evaluation results
⏳ **Pending** — run the notebook on a GPU to fill this in. This lab reports **held-out perplexity** on a held-out split (see its *Evaluate* cell).

## Inference example
No weights are published yet. After a GPU run, load the checkpoint/adapter the notebook saves (it also has a ready inference cell). Base model: **Qwen/Qwen2.5-0.5B-Instruct in 4-bit (swap any base)**.

## How to fill this repo
1. Open the [notebook in Colab](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/LM_qlora_finetune_llm.ipynb) → **Runtime → GPU → Run all** (runs the real pipeline).
...
## License
Code: **MIT** (this repository). The **base model** ([huggingface/trl + peft](https://github.com/huggingface/trl)) and **dataset** are each under their own licenses — check the upstream source before redistribution.
...
**Method / original work:** Dettmers et al., *QLoRA*, NeurIPS 2023; Hu et al., *LoRA*, 2021.

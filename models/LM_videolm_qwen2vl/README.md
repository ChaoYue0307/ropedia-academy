...
# Video-LM (Qwen2-VL)  🚧 not trained yet
...
| | |
|---|---|
| **Base model** | Qwen/Qwen2-VL-2B|7B-Instruct (pretrained) |
...
## Dataset
- **Source:** Your clip (inference); video-instruction data for LoRA.
...
## Evaluation results
⏳ **Pending** — run the notebook on a GPU to fill this in. This lab reports **Video-MME / MVBench / EgoSchema accuracy** on a held-out split (see its *Evaluate* cell).

## Inference example
No weights are published yet. After a GPU run, load the checkpoint/adapter the notebook saves (it also has a ready inference cell). Base model: **Qwen/Qwen2-VL-2B|7B-Instruct (pretrained)**.

## How to fill this repo
1. Open the [notebook in Colab](https://colab.research.google.com/github/ChaoYue0307/ropedia-academy/blob/main/notebooks/advanced/LM_videolm_qwen2vl.ipynb) → **Runtime → GPU → Run all** (runs the real pipeline).
...
## License
Code: **MIT** (this repository). The **base model** ([QwenLM/Qwen2-VL](https://github.com/QwenLM/Qwen2-VL)) and **dataset** are each under their own licenses — check the upstream source before redistribution.
...
**Method / original work:** Wang et al., *Qwen2-VL*, 2024.

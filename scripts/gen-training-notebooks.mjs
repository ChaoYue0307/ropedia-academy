...
// ---------------------------------------------------------------------------
const labs = [smplify, nerf, sdf, clip, videomae];
...
|---|---|---|---|
| ${nerf.title} | ${nerf.track} | A NeRF MLP via photometric loss (PSNR climbs) | ${badge(nerf.file)} |
| ${sdf.title} | ${sdf.track} | An MLP signed-distance field + marching-cubes surface | ${badge(sdf.file)} |
| ${smplify.title} | ${smplify.track} | Pose by reprojection optimization (+ how to use real SMPL) | ${badge(smplify.file)} |
| ${clip.title} | ${clip.track} | A linear probe on frozen CLIP features vs. zero-shot | ${badge(clip.file)} |
| ${videomae.title} | ${videomae.track} | Fine-tune VideoMAE for action recognition | ${badge(videomae.file)} |

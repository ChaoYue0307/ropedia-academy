...
export const trackA: Track = {
  ...
  lessons: [
    ...
    },
    {
      ...
      body: {
        en: "Close Track A by turning understanding into a working artifact. **Deep-dive**: take HMR or a SMPL-fitting paper and reconstruct its loss terms — reprojection, prior, and any contact/temporal terms — and explain why each exists and what fails without it. **Reproduce**: fit SMPL to a short clip's 2D keypoints via optimization (SMPLify-style), or run a pretrained mesh-recovery model and inspect failure modes (depth flips, foot-skating, penetration).\n\nThe payoff is diagnostic fluency: you can look at a wrong mesh and name which term is under-weighted. This is the same loop as Track C's Lesson 9 and Track B's Lesson 9 — read deeply, reproduce minimally, attribute errors precisely.",
        zh: "以把理解变成可运行的成果来收尾 Track A。**精读**：拿 HMR 或一篇 SMPL 拟合论文，重建其损失项——重投影、先验、以及任何接触/时间项——并解释每项为何存在、缺了它会怎样。**复现**：通过优化把 SMPL 拟合到一小段视频的 2D 关键点（SMPLify 风格），或运行预训练网格恢复模型并检查失败模式（深度翻转、脚滑、穿插）。\n\n回报是诊断的流畅度：你能看着一个错误网格，指出哪一项权重不足。这与 Track C 第 9 课、Track B 第 9 课是同一循环——深读、最小复现、精确归因误差。",
      },
      keyTerms: [

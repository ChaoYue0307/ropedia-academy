...
export const trackB: Track = {
  ...
  lessons: [
    ...
    },
    {
      ...
      body: {
        en: "Close Track B by rendering something real. **Deep-dive**: trace NeRF (or 3DGS) end-to-end — the representation, the differentiable renderer, the loss, and the engineering tricks (positional encoding, hierarchical sampling, or adaptive densification). Ask what each component buys and what removing it breaks. **Reproduce**: run an off-the-shelf NeRF/3DGS on a small captured or sample scene, then probe failure modes — too few views (floaters), bad poses (ghosting), specular surfaces.\n\nThe diagnostic skill is to read an artifact and trace it to a cause: floaters → insufficient view coverage or density regularization; blur → pose error or low capacity. This mirrors Tracks A and C: deep read, minimal reproduce, precise attribution — the loop that turns a learner into a practitioner.",
        zh: "以渲染出真实的东西来收尾 Track B。**精读**：端到端梳理 NeRF（或 3DGS）——表示、可微渲染器、损失，以及工程技巧（位置编码、分层采样或自适应稠密化）。追问每个组件买来什么、移除它会坏什么。**复现**：在一个小的自采或样例场景上跑现成的 NeRF/3DGS，然后探查失败模式——视角太少（漂浮物）、位姿差（重影）、镜面表面。\n\n诊断技能是看着一个伪影追溯到成因：漂浮物 → 视角覆盖不足或密度正则不够；模糊 → 位姿误差或容量不足。这与 Track A、C 呼应：深读、最小复现、精确归因——把学习者变为实践者的循环。",
      },
      keyTerms: [

...
export const trackC: Track = {
  ...
  lessons: [
    ...
    },
    {
      ...
      body: {
        en: "The track closes with practice, not new theory. Two exercises convert reading into capability:\n\n**Deep-dive a paper.** Pick one (e.g. an EPIC anticipation model). Don't skim — reconstruct it: *What is the exact input/output? What loss? What is the baseline it beats, and by how much? What ablation isolates the key idea? What would break it?* If you can re-derive the method block from the figures alone, you understand it.\n\n**Reproduce a small baseline.** Resist reproducing the SOTA. Build the simplest honest pipeline: features from a pretrained backbone → a small temporal head → evaluate with the official metric on a small split. The goal is a number you trust and a harness you can extend, not a leaderboard entry.\n\nThis mirrors how research actually compounds: a reproduced baseline is the platform on which your own idea becomes a controlled experiment — the bridge from *learning the field* to *doing the field*.",
        zh: "本赛道以实践收尾，而非新理论。两个练习把阅读转化为能力：\n\n**精读一篇论文。** 选一篇（如某个 EPIC 预判模型）。别略读——把它重建出来：*确切的输入/输出是什么？什么损失？它击败的基线是什么、领先多少？哪个消融分离出了关键想法？什么会让它失效？* 如果你能仅凭图就重新推导出方法部分，你就懂了。\n\n**复现一个小基线。** 抵制复现 SOTA 的冲动。搭最简单、诚实的流水线：预训练骨干取特征 → 一个小时序头部 → 在一个小划分上用官方指标评估。目标是一个你信得过的数字、一个你能扩展的脚手架，而不是榜单名次。\n\n这映射了研究真正复利的方式：一个复现的基线，是你自己的想法成为受控实验的平台——这是从*学习领域*走向*从事领域*的桥梁。",
      },
      keyTerms: [

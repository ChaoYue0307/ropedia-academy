...
export const trackD: Track = {
  ...
  lessons: [
    ...
    },
    {
      ...
      body: {
        en: "The capstone is integrative, not new theory: take a video (ideally egocentric) and run it through the pipeline you now understand — track the camera (SLAM / poses, B+D), fuse geometry (TSDF or Gaussians, B+D3), attach semantics (D4), and lift to a queryable scene graph (D5). The deliverable is a system that turns *pixels into world memory* you can query in language.\n\nThe meta-lesson is integration itself: the same SDF underlies B's NeRF and D's TSDF; B's bundle adjustment is D's SLAM back-end; C's episodic memory is a world model's memory; CLIP/LLMs bring language into every track's frontier. That web of connections — not any single method — is the difference between a researcher and a carrier of facts. Start with the simplest honest version (a few modules wired on one sequence), get a number you trust, then extend toward the cross-track frontier that excites you: drivable digital humans, semantic neural SLAM, or 4D generative world models. You've reached the summit; what's next is your own unknown.",
        zh: "终极项目是整合性的，而非新理论：取一段视频（理想是第一人称），让它流过你如今理解的流水线——跟踪相机（SLAM/位姿，B+D）、融合几何（TSDF 或高斯，B+D3）、附加语义（D4）、并提升为可查询的场景图（D5）。交付物是一个把*像素变成可用语言查询的世界记忆*的系统。\n\n元教训是整合本身：同一个 SDF 支撑 B 的 NeRF 与 D 的 TSDF；B 的光束法平差就是 D 的 SLAM 后端；C 的情景记忆就是世界模型的记忆；CLIP/LLM 把语言带进每条赛道的前沿。那张连接之网——而非任何单一方法——是研究者与事实搬运工的区别。先从最简单、诚实的版本开始（在一段序列上接几个模块），得到一个你信得过的数字，再朝你兴奋的跨赛道前沿扩展：可驱动数字人、语义神经 SLAM、或 4D 生成世界模型。你已登顶；接下来是属于你自己的未知。",
      },
      keyTerms: [
